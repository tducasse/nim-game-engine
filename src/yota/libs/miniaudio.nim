...
proc ma_sound_init_from_file*(pEngine: pointer, pFilePath: cstring;
    flags: uint32, pGroup: pointer, pFence: pointer,
...
proc ma_sound_at_end*(pSound: pointer): bool {.cdecl, importc.}
proc ma_sound_uninit*(pSound: pointer) {.cdecl, importc.}
proc ma_engine_play_sound*(pEngine: pointer,
    pFilePath: cstring): ma_result {.cdecl, importc.}
